<template>
  ...
  <input ref="inputRef" v-model="query" @input="onInput" @keydown.enter.prevent="trySelectByName" placeholder="Buscar user..." class="flex-1 bg-transparent outline-none text-sm text-black placeholder-gray-400" />
    </div>
...
// autofocus when the shared invite search becomes visible
onMounted(() => {
  if (inviteSearchVisible.value) {
